#!/usr/bin/env python3
"""
Franka Robot Pick and Place Control for Isaac Sim 4.5.0
...
It uses inverse kinematics to ensure accurate positioning of the end effector.
"""
...
def run_franka_pick_place():
    ...
    if robot is not None and joint_positions is not None:
        ...
        try:
            ...
            log(f"Banana position: {banana_position}")
            log(f"Target position: {target_position}")            # Pre-grasp position - move above the banana
            log("Moving to pre-grasp position...")
            pre_grasp_offset = np.array([0.0, 0.0, 0.03])  # 3cm above the banana (reduced from 5cm)
            pre_grasp_joints = calculate_ik_for_position(
                robot, 
                banana_position, 
                offset=pre_grasp_offset, 
                gripper_open=True,
                orientation='horizontal-ew'  # Orient gripper east-west for banana
            )
            ...
            grasp_offset = np.array([0.0, 0.0, -0.04])  # 4cm below the banana's center point (was 2cm)
            grasp_joints = calculate_ik_for_position(
                robot, 
                banana_position, 
                offset=grasp_offset, 
                gripper_open=True,
