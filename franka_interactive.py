#!/usr/bin/env python3
"""
Franka Robot Interactive Control in Isaac Sim 4.5.0
...
- Display of current joint positions
"""
...
def run_franka_interactive_control():
    ...
    if robot is not None and joint_positions is not None and dof_names is not None:
        ...
        window = ui.Window("Franka Robot Control", width=500, height=800)
        with window.frame:
            with ui.VStack(spacing=5, height=0):
                ...
                # Joint positions display with a frame around it
                with ui.Frame(height=0, style={"border_width": 1, "border_color": 0xFF999999, "background_color": 0xFF333333}):
                    with ui.VStack(spacing=2, style={"margin": 10}):
                        ui.Label("Current Joint Positions:", style={"font_size": 16})
                        position_labels = []
                ...
                joint_label = ui.Label(f"Selected Joint: {dof_names[State.selected_joint]}", 
                                       height=30, 
                                       style={"font_size": 18, "color": 0xFF66CCFF})
                ...
                with ui.VStack(spacing=5):
                    ...
                    with ui.HStack(spacing=5):
                        for i in range(1, 5):
                            ...
                            ui.Button(f"Joint {i}", width=110, height=30, 
                                      clicked_fn=make_select_callback(i-1),
                                      style={"button_color": 0xFF4477AA})
                    ...
                    with ui.HStack(spacing=5):
                        ...
                        def select_fingers():
                            ...
                            joint_label.text = f"Selected: Gripper"
                            log(f"Selected both finger joints: {dof_names[7]} and {dof_names[8]}")
                        
                        ui.Button("Gripper", width=110, height=30, 
                                 clicked_fn=select_fingers,
                                 style={"button_color": 0xFF4477AA})
                ...
                with ui.VStack(spacing=10):
                    ui.Label("Joint Movement Controls:", style={"font_size": 16})
                    ...
                    with ui.HStack(spacing=10):
                        ...
                        ui.Button("⬆️ Increase", width=225, height=50, 
                                 clicked_fn=increase_joint,
                                 style={"button_color": 0xFF55AA55, "font_size": 16})
                        ui.Button("⬇️ Decrease", width=225, height=50, 
                                 clicked_fn=decrease_joint,
                                 style={"button_color": 0xFFAA5555, "font_size": 16})
                ...
                ui.Button("Reset All Joints", width=450, height=40, 
                         clicked_fn=reset_joints,
                         style={"button_color": 0xFFAA7722, "font_size": 16})
                
                ui.Spacer(height=10)
                
                # Step size control
                with ui.VStack(spacing=5):
                    ui.Label("Movement Step Size:", style={"font_size": 16})
                    ...
                    with ui.HStack(spacing=5):
                        ...
                        ui.Button("Small (0.01)", width=145, height=30, 
                                 clicked_fn=lambda: set_step_size(0.01),
                                 style={"button_color": 0xFF555599})
                        ui.Button("Medium (0.05)", width=145, height=30, 
                                 clicked_fn=lambda: set_step_size(0.05),
                                 style={"button_color": 0xFF555599})
                        ui.Button("Large (0.1)", width=145, height=30, 
                                 clicked_fn=lambda: set_step_size(0.1),
                                 style={"button_color": 0xFF555599})
                
                ui.Spacer(height=10)
                
                # Instructions
                with ui.Frame(style={"border_width": 1, "border_color": 0xFF999999, "background_color": 0xFF333333}):
